...
public class AvailableAppsLoader {
    private final GitHubInfo m_gitHubInfo;
    private final ExecutorService m_execService;
    
    public AvailableAppsLoader(GitHubInfo gitHubInfo, ExecutorService execService) {
        m_gitHubInfo = gitHubInfo;
        m_execService = execService;
    }
    
    public CompletableFuture<List<AppInformation>> loadAvailableApps() {
    ...
    private List<AppInformation> parseAvailableApps(JsonObject availableAppsJson) {
        List<AppInformation> apps = new ArrayList<>();
        ...
        if (appsElement != null && appsElement.isJsonArray()) {
            ...
            for (JsonElement appElement : appsArray) {
                if (appElement.isJsonObject()) {
                    ...
                    try {
                        AppInformation appInfo = parseAppInfo(appObj);
                        if (appInfo != null) {
                            apps.add(appInfo);
                        }
                    } catch (Exception e) {
    ...
    private AppInformation parseAppInfo(JsonObject appObj) {
        // Parse basic info
        ...
        Image icon = iconUrl != null ? new Image(iconUrl) : null;
        Image smallIcon = smallIconUrl != null ? new Image(smallIconUrl) : null;
        ...
        return new AppInformation(appId, appName, icon, smallIcon, description, 
