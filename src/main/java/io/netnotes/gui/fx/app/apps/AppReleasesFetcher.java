...
public class AppReleasesFetcher {
    private final ExecutorService m_execService;
    
    public AppReleasesFetcher(ExecutorService execService) {
        m_execService = execService;
    }
    
    public CompletableFuture<List<AppRelease>> fetchReleasesForApp(AppInformation appInfo) {
        ...
        return api.getAssetsAllLatestRelease(m_execService)
            .thenApply(assets -> {
                List<AppRelease> releases = new ArrayList<>();
                        ...
                        for (GitHubFileInfo ghFile : gitHubFiles) {
                            if (assetMatchesFileInfo(asset, ghFile)) {
                                String version = extractVersion(asset.getName(), 
                                                               ghFile.getFileName(), 
                                                               ghFile.getFileExt());
                                AppRelease release = new AppRelease(appInfo, asset, version, 
                                                                    asset.getTagName(), 
